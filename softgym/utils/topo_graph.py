...
if __name__ == '__main__':
    ...
    #     for c in [-1,1]:
    #         for s in [-1,1]:
    #             new_topo, segs = topo.add_R1(test_topo,i,c,s)
    #             print(f'{i},{c},{s}')
    #             print(new_topo)
    ...
    f, (ax1,ax2) = plt.subplots(1,2)
    for sign in [-1,1]:
        for over in range(test_topo.size()+1):
            if over in [0,test_topo.size()]:
                range = range(test_topo.size()+1)
            else:
                range = [0,test_topo.size()]

            for under in range:
                if over == under:
                    continue
                new_topo, segs = topo.add_C(test_topo,over,under,sign)
                print(f'{over},{under},{sign}')
                print(new_topo)

                try:
                    test_topo.display(ax1,over,under,sign)
                    new_topo.display(ax2,segs)
                    plt.pause(5)
                    ax1.clear()
                    ax2.clear()
